...
namespace Core.CrossCuttingConcerns.Logging.Log4Net
{
    public class LoggerServiceBase
    {
        ...
        {
            if (this.IsInfoEnabled) this._log.Debug(logMessage);
        }
        public void Warn(object logMessage)
        {
            if (this.IsInfoEnabled) this._log.Warn(logMessage);
        }
        public void Fatal(object logMessage)
        {
            if (this.IsInfoEnabled) this._log.Fatal(logMessage);
        }
        public void Error(object logMessage)
        {
            if (this.IsInfoEnabled) this._log.Error(logMessage);
        }
